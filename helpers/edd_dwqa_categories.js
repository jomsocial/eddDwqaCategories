...
//@EDD_DWQA - start
//console.log($("#dwqa-question_category_parent :selected").attr('value'));
//console.log($("#dwqa-question_category_child :selected").attr('value'));

		if($("#dwqa-question_category_child :selected").attr('value')){
			category = $("#dwqa-question_category_child :selected").attr('value');
		}else if($("#dwqa-question_category_parent :selected").attr('value')){
			category = $("#dwqa-question_category_parent :selected").attr('value');
		}else{
			category = 'all';
		}
...
function edd_dwqa_populate_question_category(){
	var $JS		= jQuery.noConflict();
	// Get the value from a dropdown select even easier
	var selected_child 	= $JS( "select#dwqa-question_category_child" ).val();
	var selected_parent = $JS( "select#dwqa-question_category_parent" ).val();	

	if(selected_child){
		$JS( "input#question-category" ).val(selected_child);
	}else{
		$JS( "input#question-category" ).val(selected_parent);		
	}
	return true;
}
...
function checkUserProduct(){
	var $JS		= jQuery.noConflict();
	var cat_id	= document.getElementById('question-category').value;
	...
	$JS.ajax({
		...
		type: 'post',
		dataType: 'json',
		data: ({action : 'checkUserProduct', cat_id : cat_id}),			
		success: function(response){
			//console.log(response);
			if(response.error){
				if(response.error == 'purchased_expired'){
					//display error msg
					$JS('#checkUserProductMessage').html('Your license to this product has expired. Please renew it to access support. Go to "My Orders", select your order and then click the Renew button.');
				}else{
					//display error msg
					$JS('#checkUserProductMessage').html('You don\'t have license to this product, please visit our store to purchase it');
				}
				$JS('#question-title').attr("disabled", true); 				
				$JS('#sinfo_site_url').attr("disabled", true);
				$JS('#sinfo_site_adminu').attr("disabled", true);
				$JS('#sinfo_site_adminp').attr("disabled", true);
				$JS('#sinfo_ftp_host').attr("disabled", true);
				$JS('#sinfo_ftp_u').attr("disabled", true);
				$JS('#sinfo_ftp_p').attr("disabled", true);
				$JS('#sinfo_misc').attr("disabled", true);
				$JS('#sinfo_dev_url').attr("disabled", true);
				$JS('#sinfo_dev_adminu').attr("disabled", true);
				$JS('#sinfo_dev_adminp').attr("disabled", true);
				$JS('#sinfo_htaccess_u').attr("disabled", true);
				$JS('#sinfo_htaccess_p').attr("disabled", true);
				$JS('#select-userexpertise').attr("disabled", true);
				$JS('.btn-submit-question').attr("disabled", true);
				
				$JS('#site_info_user_domains').html('Please enter your support domain first. Visit My Orders, select the order and on the license page enter the domain and save. Then come back to this page.');
				...
				$JS.ajax({
					...
					type: 'post',
					dataType: 'json',
					data: ({action : 'getUserDomains', cat_id : cat_id}),			
					success: function(response){
						console.log(response);
						if(response.error){
							$JS('#site_info_user_domains').html('Please enter your support domain first. Visit My Orders, select the order and on the license page enter the domain and save. Then come back to this page.');
							...
							$JS('#site_info_user_domains').html(response.message);
							$JS('#sinfo_site_url').attr("disabled", false);
							$JS('#sinfo_site_adminu').attr("disabled", false);
							$JS('#sinfo_site_adminp').attr("disabled", false);
							$JS('#sinfo_ftp_host').attr("disabled", false);
							$JS('#sinfo_ftp_u').attr("disabled", false);
							$JS('#sinfo_ftp_p').attr("disabled", false);
							$JS('#sinfo_misc').attr("disabled", false);
							$JS('#sinfo_dev_url').attr("disabled", false);
							$JS('#sinfo_dev_adminu').attr("disabled", false);
							$JS('#sinfo_dev_adminp').attr("disabled", false);
							$JS('#sinfo_htaccess_u').attr("disabled", false);
							$JS('#sinfo_htaccess_p').attr("disabled", false);
							$JS('#select-userexpertise').attr("disabled", false);
							$JS('.btn-submit-question').attr("disabled", false);
						}
					}
				});				
				
				$JS('#checkUserProductMessage').html('');
				$JS('#question-title').attr("disabled", false); 
			}
		}
	});	
}